...
def check_triangles(possible_location, play):
    ...
    height = total_height - int(possible_location[1]) - 1
...
def find_moves(current_location, play):
    ...
    height = total_height - int(current_location[1]) - 1
    dist_left = int(current_location[2])
    dist_right = int(current_location[3])
    #row = sys.argv[1][height]
    
    #colors for each of the possible surrounding locations    
    #print(play[height]) #should print the row that the last play was made
    #print(play[height][dist_left])    
    #print(height)
    ...
    upper_left = play[height - 1][dist_left - 1]
    upper_right = play[height - 1][dist_left]
    bottom_left = play[height + 1][len(play[height - 1]) - dist_left]
    bottom_right = play[height + 1][dist_right - 1]
    ...
    if (left_side == "0"):
        left_move = ["0", str(height), str(dist_left - 1), str(dist_right + 1)]
        possible_moves.append(left_move)
    
    #get position of right side
    if (right_side == "0"):
        right_move = ["0", str(height), str(dist_left + 1), str(dist_right - 1)]
        possible_moves.append(right_move)    
    
    if (upper_left == "0"):
        upper_left_move = ["0", str(height + 1), str(dist_left - 1), str(dist_right)]
        possible_moves.append(upper_left_move)
    
    if (upper_right == "0"):
        upper_right_move = ["0", str(height + 1), str(dist_left), str(dist_right - 1)]
        possible_moves.append(upper_right_move)
    
    if (bottom_left == "0"):
        bottom_left_move = ["0", str(height - 1), str(dist_left), str(dist_right + 1)]
        possible_moves.append(bottom_left_move)
    
    if (bottom_right == "0"):
        bottom_right_move = ["0", str(height - 1), str(dist_left + 1), str(dist_right)]
...
def minimax(current_move, depth, maximizingPlayer):
    ...
    if (depth == 0 or len(find_moves(current_move, play)) == []):
    ...
    if maximizingPlayer:
        ...
        best_value = -10000
        possible_moves = find_moves(current_move, play)
...
if (last_play[-1] == "null"):
   ...
   # print("Last play location", last_play_location)
    ...
    if (possible_moves == []):
        ...
        for i in range(len(open_moves)):
                
            total_moves.append(open_moves[i])
        ...
        for j in range(len(total_moves)):
                ...
                #print(j)
                triangle_checks.append(check_triangles(total_moves[j], play))
                ...
                if (len(triangle_checks[j]) == 3):
                    ...
                    minimax_scores.append(minimax(total_moves[j], depth, True))
        ...
        if (good_moves != []):
            ...
            if (colors != []):
                ...
                sys.stdout.write(str((int(final_move[0]), int(final_move[1]), int(final_move[2]), int(final_move[3]))));
            ...
            if (colors != []):
                ...
                final_move = [colors[0]] + make_move[1:]
                    ...
                    #print(str(final_move[0], final_move[1], final_move[2], final_move[3]))
        ...
        if (len(possible_moves) == 1):
            ...
            losing_colors = check_triangles(possible_moves[0], play)
            ...
            if ((len(losing_colors) == 3)):
                ...
                (str(final_move[0], final_move[1], final_move[2], final_move[3]))                
